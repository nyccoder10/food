...
import { RES_CARD_IMG_CDN_URL } from "./constant";
...
//   const [state, setState] = useState("");
  ...
  const handleDecreaseQuantity = (id) => dispatch(decreaseItemQuantity({ id }));
  const handleIncreaseQuantity = (id) => dispatch(increaseItemQuantity({ id }));
  ...
  },[user.user.loc,orderSuccess])

  return cartDetails.length === 0 ? (
    <div className="flex flex-col items-center justify-start min-h-screen">
      <img src={emptyCart} className="w-auto mt-10 h-[40vh]"></img>
      ...
      <Link
        to="/"
        className="px-4 py-2 bg-[#1C1572] text-white font-bold mb-4 hover:shadow-md"
      >
        SEE RESTAURANTS NEAR YOU
      </Link>
    </div>
  ) : (
    orderSuccess ? <Success /> : <div className="flex flex-col  bg-[#e9ecee] min-h-screen">
      <div className="flex mx-auto mt-4 xl:w-[85%] lg:w-[85%] md:w-[96%] sm:w-[96%]">
            ...
            <div className="max-h-[67vh] overflow-y-auto">
              <div className="flex flex-col mx-6 pt-6">
                {cartDetails.map((item) => {
                  return (
                    <div className="flex w-full justify-between px-2 mb-3 last:mb-0 items-center" key={item?.item?.card?.info?.id}>
        ...
        {/*                         
                          ...
                          <div className="">
                    <img
                      className="w-16 h-16  object-cover block rounded-md aspect-square"
                    ...
                    />
                      ...
                      <div className="flex border-[1.11px] border-solid border-gray-300 p-1 text-sm items-center">
                        <div className="px-2 font-bold flex-1 text-[#3e4152] cursor-pointer" onClick={() => {handleDecreaseQuantity(item?.card?.info?.id)}}>-</div>
                        <div className="px-2 font-bold flex-1 text-[#60b246] text-xs">{item.quantity}</div>
                        <div className="px-2 font-bold flex-1 text-[#60b246] cursor-pointer" onClick={() => {handleIncreaseQuantity(item?.card?.info?.id)}}>+</div>
                        
                      </div>
                      <div>
                        
                        <h3 className="text-[#686b78] text-xs">₹{item.quantity * (item.itemPrice)/100}</h3>
                       
                      </div>
                     <button onClick={() => removeItem(item?.card?.info?.id)} ><FontAwesomeIcon icon={faTrash} /> </button> 
                    </div>
                  );
                })}
              </div>
              ...
              <div className="mx-4 p-2">
                ...
                <div className="flex justify-between text-[#686b78] text-xs font-normal pt-4">
                  <h3>Item Total</h3>
                  <h3 className="text-nowrap">₹ {totalPrice/100}</h3>
                </div>
                <div className="flex justify-between border-b border-solid text-[#686b78] text-xs font-normal pb-4 pt-1">
                  ...
                  <h3 className="text-nowrap">+ ₹ {parseFloat(deliveryCharges).toFixed(2)}</h3>
                </div>
              </div>
            <div className="mx-4 p-2">
            ...
            <div className="flex justify-between text-[#282c3f] text-sm font-extrabold mx-4 px-2 my-2 tracking-tight">
              ...
              <h3 className="text-nowrap">
  ₹{" "}
  {(totalPrice / 100 + totalPrice * 0.18 / 100 + 3 + parseFloat(deliveryCharges)).toFixed(2)}
</h3>
